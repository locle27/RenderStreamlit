...
from logic import (
    ...
    # Các hàm CRUD không cần import trực tiếp nếu đã có route xử lý
)
...
TOTAL_HOTEL_CAPACITY = 4 # SỬA LỖI: Cập nhật đúng tổng số phòng
...
def load_data():
    ...
    try:
        if not GCP_CREDS_DICT: raise ValueError("Credentials Google Cloud không hợp lệ")
        df = import_from_gsheet(DEFAULT_SHEET_ID, GCP_CREDS_DICT, WORKSHEET_NAME)
        if df.empty: raise ValueError("Sheet trống, không có dữ liệu để tải.")
        active_bookings = df[df['Tình trạng'] != 'Đã hủy'].copy()
        return df, active_bookings
    except Exception as e:
        print(f"Lỗi tải dữ liệu: {e}. Sử dụng dữ liệu demo.")
